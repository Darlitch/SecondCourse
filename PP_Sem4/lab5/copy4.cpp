...
void* Getter() {
	...
	while (refusalCounter < size - 1) {
		int wantWork = -1;
		MPI_Status status;
		MPI_Recv(&wantWork, 1, MPI_INT, MPI_ANY_SOURCE, 1, MPI_COMM_WORLD,
				 &status);
		...
		if (counter < localWorkCounter - 1) {
			...
			MPI_Send(startShared, sharedWork, MPI_INT, sendRank, 0,
					 MPI_COMM_WORLD);
...
void GetWork(int* visited) {
	...
	for (int i = 0; i < size; ++i) {
		if (visited[i] != 1) {
			...
			int countNewWork;
			MPI_Recv(&countNewWork, 1, MPI_INT, i, 0, MPI_COMM_WORLD,
					 MPI_STATUS_IGNORE);
			if (countNewWork == -1) {
			...
			MPI_Recv(localWorks, countNewWork, MPI_INT, i, 0, MPI_COMM_WORLD,
					 MPI_STATUS_IGNORE);
...
void* Work() {
	...
	while (localWorkCounter > 0) {
		while (counter < localWorkCounter) {
			...
			// localWorks[counter], counter + 1, localWorkCounter );
...
void SharedWork() {
	...
	MPI_Scatterv(works, counts, shift, MPI_INT, localWorks, counts[rank],
